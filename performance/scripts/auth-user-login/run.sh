...
$COMMON/_locust-log-to-csv.sh 'loginUserAuth open-login-page-time' $JOB_BASE_NAME-$BUILD_NUMBER-locust-master.log
$COMMON/_locust-log-to-csv.sh 'loginUserAuth login-time' $JOB_BASE_NAME-$BUILD_NUMBER-locust-master.log

echo " Generate charts from CSV"
export REPORT_CHART_WIDTH=1000
export REPORT_CHART_HEIGHT=600
for c in $(find *.csv | grep '\-loginUserAuth_\+\|\-loginUserOAuth2_\+'); do echo $c; $COMMON/_csv-response-time-to-png.sh $c; $COMMON/_csv-throughput-to-png.sh $c; $COMMON/_csv-failures-to-png.sh $c; done
...
for c in $(find *.csv | grep '\-report_distribution.csv'); do
	distribution_2_csv $c '"loginUserAuth open-login-page-time"';
	distribution_2_csv $c '"loginUserAuth login-time"';
done
...
filterZabbixValue $ZABBIX_LOG "login-user.auth.open-login-page-rt_min" "@@OPEN_LOGIN_PAGE_TIME_MIN@@" $RESULTS_FILE;
filterZabbixValue $ZABBIX_LOG "login-user.auth.open-login-page-rt_median" "@@OPEN_LOGIN_PAGE_TIME_MEDIAN@@" $RESULTS_FILE;
filterZabbixValue $ZABBIX_LOG "login-user.auth.open-login-page-rt_max" "@@OPEN_LOGIN_PAGE_TIME_MAX@@" $RESULTS_FILE;
filterZabbixValue $ZABBIX_LOG "login-user.auth.open-login-page-rt_average" "@@OPEN_LOGIN_PAGE_TIME_AVERAGE@@" $RESULTS_FILE;
filterZabbixValue $ZABBIX_LOG "login-user.auth.open-login-page-failed" "@@OPEN_LOGIN_PAGE_TIME_FAILED@@" $RESULTS_FILE;

filterZabbixValue $ZABBIX_LOG "login-user.auth.login-rt_min" "@@LOGIN_TIME_MIN@@" $RESULTS_FILE;
filterZabbixValue $ZABBIX_LOG "login-user.auth.login-rt_median" "@@LOGIN_TIME_MEDIAN@@" $RESULTS_FILE;
filterZabbixValue $ZABBIX_LOG "login-user.auth.login-rt_max" "@@LOGIN_TIME_MAX@@" $RESULTS_FILE;
filterZabbixValue $ZABBIX_LOG "login-user.auth.login-rt_average" "@@LOGIN_TIME_AVERAGE@@" $RESULTS_FILE;
filterZabbixValue $ZABBIX_LOG "login-user.auth.login-failed" "@@LOGIN_TIME_FAILED@@" $RESULTS_FILE;
